...
Key technical decisions and their trade-offs are documented in [`/docs/adr`](docs/adr/).

| ADR | Decision |
|-----|----------|
| [ADR-001](docs/adr/ADR-001.md) | Why GPT-4o function calling over LangChain |
| [ADR-002](docs/adr/ADR-002.md) | Why pytest subprocess over importlib |
| [ADR-003](docs/adr/ADR-003.md) | Why Render over Railway or Fly.io |

---

## Known Limitations

- **Python only** — JavaScript and TypeScript support is on the roadmap.
- **Single-file bugs** — Rover does not yet handle bugs that require coordinated changes across more than two files.
- **Render free tier** — the server spins down after 15 minutes of inactivity; the first webhook after idle may take ~30 seconds to respond.
- **Tool call limit** — capped at 20 tool calls per run as a safety measure.

---

## Roadmap

- [ ] JavaScript / TypeScript support (ESLint + Jest)
- [ ] Regression protection — abort PR if existing tests break
- [ ] Slack webhook notifications on completion
- [ ] PostgreSQL-backed run history (replacing JSON log files)
- [ ] AST call graph for multi-file dependency awareness
- [ ] Fine-tuned model on collected Rover run data

---

## Contributing

Contributions are welcome. See [CONTRIBUTING.md](CONTRIBUTING.md) for setup instructions and code style guidelines.
